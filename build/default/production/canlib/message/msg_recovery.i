...
# 3 "canlib/message/msg_recovery.c" 2
...
# 8 "canlib/message/msg_recovery.c" 2
...
_Bool build_alt_arm_cmd_msg(
    ...
    output->sid = (((uint32_t)prio << 27) | ((uint32_t)MSG_ALT_ARM_CMD << 18) | ((uint32_t)BOARD_TYPE_ID_CANARD_MOTOR << 8) | BOARD_INST_ID_CANARD_MOTOR_PRIMARY);
...
_Bool build_alt_arm_status_msg(
    ...
    output->sid = (((uint32_t)prio << 27) | ((uint32_t)MSG_ALT_ARM_STATUS << 18) | ((uint32_t)BOARD_TYPE_ID_CANARD_MOTOR << 8) | BOARD_INST_ID_CANARD_MOTOR_PRIMARY);
    write_timestamp_2bytes(timestamp, output);
